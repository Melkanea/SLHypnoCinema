...
debug(string txt)
{
    ...
    llSay(DEBUG_CHANNEL, "Owner-Key: "+(string)owner);
    llSay(DEBUG_CHANNEL, "COUNTER: "+(string)counter);
    llSay(DEBUG_CHANNEL, "Victim-Key: "+(string)victim);
...
next() //process next entry in queue
{
 ...
 //=====================Melkaneas Snippet====================//
 getInfo(string coordinates) // Get the Vector of the object
 {
     key target = llGetKey();
...
restart(string erase)
{
    if (sitting == FALSE)
    {
        ...
        owner = NULL_KEY;
        llSetTimerEvent(0.0);
        llSay(0, "Total amount of collisions"+(string)collision_total);
        }
}
...
default
{
    ...
    collision_start(integer num)
    {
        ...
        key owner = llGetOwner(); //RLV User to Teleport.
    ...
    {
    getInfo(""); // Get the Vector of the object
    collision_total++;
    }
